...
name: issue-sweep
description: >-
  Batch-resolve open local issues. Scan ai-memories/issues/ for status:open, assess
...
阶段性把 `ai-memories/issues/` 里 `open` 的 issue 打包解决，践行"批量处理优于单点突破、避免过度开发"。

配套理念见 [`ai-memories/issues/README.md`](../../../ai-memories/issues/README.md)；触发时机与规则见 [`.cursor/rules/workflow/ai-workflow.mdc`](../../rules/workflow/ai-workflow.mdc)。
...
### Step 1 — 扫描（只读）
- 列出 `ai-memories/issues/issue-*.md` 中 `status: open` 的文件。
- 读取每个的 frontmatter（id, priority, tags, created, related）与标题、现象/期待。
- 同时可读近期 `ai-memories/tasks/` Plan 与 `git log` 了解上下文（可选）。
...
- Plan 中「关联」字段必须列出本批所有 issue 路径（如 `ai-memories/issues/issue-0001-*.md`）。
...
- 对每个已解决的 issue：
  ...
  - 在 frontmatter 追加：
    ```
    closed: <YYYY-MM-DD>
    resolved-by: ai-memories/tasks/<本次plan>.md
    ```
...
- 若本次修复沉淀了可复用的模式、坑或决策，调用 `memory-update` skill 增量刷新 `ai-memories/memories/`。
- 否则仅提交即可，避免不必要的记忆更新。
...
- → 交由 task-plan 生成 Plan：ai-memories/tasks/YYYY-MM-DD-<slug>.md
