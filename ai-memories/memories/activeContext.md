...
> 2026-07-31: Deployed and accepted fixed candidate `5935b84` for the navigation/content cleanup. The stable left brand, locale-aware fluid desktop navigation, reversed projects, 0731 ISO/CE assets, and Contact form removal are live. Typecheck, 24 tests, build, rollback archive, Nginx, eight public routes, exact local/remote dist identity, OSS checks, and live Chrome checks pass.
...
> 2026-07-26: Completed GitHub #12 partner-logo showcase foundation. No partner name or Logo has public approval, so the reusable grouped grid exposes four neutral `pending-replacement` slots with visible three-language labels. Future images are restricted to `cdnUrl()` OSS assets with localized alt text; typecheck, contract tests, and production build pass.
...
> 2026-07-26: Completed GitHub #7 Industry Applications: six GREENGAS-supported sectors now have multilingual icon navigation and structured environment, challenge, response, equipment, and related-case sections. Detailed records and case associations are visibly marked Example Placeholder; product/case links use stable IDs and base-safe URLs. Typecheck, tests, and root/subpath builds pass.
...
甲方新增导航、案例顺序、高清证书与 Contact 清理固定候选 `5935b84` 已提交、推送、部署并通过线上验收。域名解析继续等待公安备案完成。

## 近期变更

- 2026-07-31 导航与内容清理生产发布：固定候选 `5935b84` 已上线；稳定品牌排版、三语差异化导航断点与流式字号、代表性项目反序、0731 高清证书及 Contact 表单删除通过验收。回滚包、Nginx、8 路由、OSS、本地/生产产物一致性及在线 Chrome 检查通过。
...
- 2026-07-26 行业应用：建立钢铁、化工、电力、制药、军工、特种设施六类三语行业入口；每类展示运行环境、挑战、应对、推荐设备和相关案例空间，示例状态明确，跨页链接使用稳定 ID 与 `basePath()`。
...
- `ai-memories/tasks/2026-07-26-industry-applications.md`（GitHub #7 已完成，详细资料待审批替换）
