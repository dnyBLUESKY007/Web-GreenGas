...
- [x] Step 6: 推送候选，备份当前生产站，部署固定提交并完成线上验收。
- [x] Step 7: 记录发布结果，关闭全部执行 Issue 和总 Issue；在总结中向甲方询问浩达项目地点。
...
- 2026-07-27 部署前保存原生产 `dist` 到 `ignored/deployment-rollbacks/2026-07-27-pre-issue-23-dist.tar.gz`（SHA-256 `2fd890…eef5`）。清理 iCloud 生成目录重复项和陈旧 `dist` 后，从锁文件重建并发布固定 47 文件候选；Nginx、八路由、20 组在线浏览器检查、三语、ISO、标题、页脚、地图、23 张 OSS 图片及本地/远端文件聚合全部通过。

## Lessons Learned

- 静态候选必须从空 `dist/` 生成；Vite 的正常清理不能替代部署前对文件数和摘要的独立核对。
- iCloud 同步目录可能为生成依赖创建带数字后缀的冲突副本；出现异常隐式类型目录时应清理 `node_modules` 并按锁文件重装，不能修改源码规避。
- 本地与 Linux 远端聚合文件清单时必须固定 `LC_ALL=C` 或使用相同排序规则，否则内容相同也会因大小写排序差异产生不同聚合摘要。
