...
- 2026-07-26: 收尾审查发现通用阿里云 OSS 域名规则会信任非 GREENGAS bucket；通过 RED/GREEN 契约测试改为仅允许 `CDN_BASE` 配置的主机，并补充分类/资料 ID 唯一性、筛选分组语义和下载链接可访问名称约束。
...
- “阿里云 OSS”不是充分的来源边界；公开下载必须限制到项目配置的 GREENGAS bucket，避免误信任其他租户。
